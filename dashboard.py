...
INNER JOIN users ON billdeliverydev_check.postedby = users.id
JOIN areaoffice on areaoffice.aoid = users.AreaOffice
WHERE billdeliverydev_check.trans_date between "2023-01-01" and "2023-12-31" AND billdeliverydev_check.status = "delivered"
GROUP BY AreaOffice 
...
# start_date = st.date_input("Start Date", value=pd.to_datetime("2023-01-01", format="%Y-%m-%d"))
...
#
# st.table(ao_data2.loc[start:end])
...
bill_data1 = pd.read_sql_query("""SELECT latitude, longitude FROM billdeliverydev_check """, conn)
